...
def evaluate_on_test(model_config_path, data_config_path, trainer_config_path):
    ...
    logging.info(f"--- Starting Final Evaluation on TEST SET for: {exp_name} ---")
    ...
    if not os.path.exists(test_csv_path):
        logging.error(f"Test data not found at {test_csv_path}. Please run the preprocessing script first to generate data splits.")
        return
    ...
    test_dataset = MultiModalStrokeDataset(csv_path=test_csv_path, **dataset_params)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Evaluate a trained multi-modal model on the test set.")
    ...
    evaluate_on_test(args.model, args.data, args.trainer)